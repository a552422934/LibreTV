...
// 源站检测结果本地缓存：键名与有效期（1 天）
const SITE_AVAILABILITY_CACHE_KEY = 'siteAvailabilityCache';
const SITE_AVAILABILITY_CACHE_TTL = 24 * 60 * 60 * 1000; // 1 天（毫秒）
...
// 剔除已从配置中移除的源，避免请求时报「无效的API来源」
function sanitizeSelectedAPIs() {
    const valid = selectedAPIs.filter(id => {
        if (id.startsWith('custom_')) {
            const idx = parseInt(id.replace('custom_', ''), 10);
            return !isNaN(idx) && idx >= 0 && idx < customAPIs.length;
        }
        return window.API_SITES && Object.prototype.hasOwnProperty.call(window.API_SITES, id);
    });
    ...
    Object.keys(API_SITES).forEach(apiKey => {
        ...
        checkbox.innerHTML = `
            <input type="checkbox" id="api_${apiKey}" 
                   ...
                   ${checked ? 'checked' : ''} 
                   data-api="${apiKey}">
            <label for="api_${apiKey}" class="ml-1 text-xs text-gray-400 truncate flex items-center">
                <span>${api.name}</span>
                <span class="ml-1 text-[10px] api-status" data-api-status="${apiKey}"></span>
            </label>
        `;
        ...
        Object.keys(API_SITES).forEach(apiKey => {
            ...
            checkbox.innerHTML = `
                <input type="checkbox" id="api_${apiKey}" 
                       ...
                       ${checked ? 'checked' : ''} 
                       data-api="${apiKey}">
                <label for="api_${apiKey}" class="ml-1 text-xs text-pink-400 truncate flex items-center">
                    <span>${api.name}</span>
                    <span class="ml-1 text-[10px] api-status" data-api-status="${apiKey}"></span>
                </label>
            `;
...
// 从本地存储读取源站检测缓存（仅当未过期时有效）
function getSiteAvailabilityCache() {
    try {
        const raw = localStorage.getItem(SITE_AVAILABILITY_CACHE_KEY);
        if (!raw) return null;
        const data = JSON.parse(raw);
        if (!data || typeof data.timestamp !== 'number' || !data.results || typeof data.results !== 'object') return null;
        if (Date.now() - data.timestamp > SITE_AVAILABILITY_CACHE_TTL) return null;
        return data.results; // { apiKey: true/false, ... }
    } catch (e) {
        return null;
    }
}
...
// 将检测结果写入本地存储，有效期 1 天
function setSiteAvailabilityCache(results) {
    try {
        localStorage.setItem(SITE_AVAILABILITY_CACHE_KEY, JSON.stringify({
            results: results,
            timestamp: Date.now()
        }));
    } catch (e) {
        console.warn('保存源站检测缓存失败:', e);
    }
}

// 页面加载时根据缓存恢复 ✔/✖ 显示（仅当缓存未过期）
function applySiteStatusFromCache() {
    const results = getSiteAvailabilityCache();
    if (!results) return;
    document.querySelectorAll('#apiCheckboxes .api-status').forEach(span => {
        const key = span.getAttribute('data-api-status');
        if (!key || results[key] === undefined) return;
        const available = !!results[key];
        span.textContent = available ? '✔' : '✖';
        span.classList.toggle('text-green-400', available);
        span.classList.toggle('text-red-400', !available);
    });
}

// 批量检查内置源站可用性并在列表中标记对号/叉号，结果写入本地存储（有效期 1 天）
async function checkBuiltInSitesAndMark() {
    if (typeof testSiteAvailability !== 'function' || !window.API_SITES) {
        showToast && showToast('检测函数未加载，请稍后重试', 'error');
        return;
    }

    try {
        showToast && showToast('正在检查内置源站可用性，请稍候...', 'info');
        // 清空之前的状态图标
        document.querySelectorAll('#apiCheckboxes .api-status').forEach(span => {
            span.textContent = '';
        ...
        const cacheResults = {}; // 用于写入本地存储

        // 逐个源依次检测并立刻在 UI 中标记结果
        const entries = Object.entries(window.API_SITES);
        for (const [key, info] of entries) {
            const apiUrl = info.api;
            if (!apiUrl || !/^https?:\/\//i.test(apiUrl)) continue;

            const span = document.querySelector(`#apiCheckboxes .api-status[data-api-status="${key}"]`);
            if (span) {
            ...
            let available = false;
            try {
                available = await testSiteAvailability(apiUrl);
            } catch (e) {
                available = false;
            }

            cacheResults[key] = available;
        ...
        setSiteAvailabilityCache(cacheResults);
        showToast && showToast('源站检查完成，可根据标记调整选择', 'success');
    } catch (e) {
        console.error('检查源站可用性失败:', e);
        showToast && showToast('检查源站可用性失败，请稍后重试', 'error');
    }
}
