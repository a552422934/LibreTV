...
// 切换资源列表：会话缓存（进入页面即加载，展开时直接用缓存）
let switchResourceCache = { videoKey: '', html: '', loading: false };
...
function initializePageContent() {
    ...
    // 进入页面即后台加载切换资源列表，会话内缓存
    loadSwitchResourceListInBackground();
...
function renderResourceInfoBar() {
    ...
    container.innerHTML = `
      ...
      <button class="resource-switch-btn flex" id="switchResourceBtn" onclick="toggleSwitchResourceExpand()">
...
// 切换资源区域：展开/收起（仅展示，内容来自进入页面时加载的会话缓存）
function toggleSwitchResourceExpand() {
    const el = document.getElementById('switchResourceExpand');
    const container = document.getElementById('switchResourceExpandContent');
    ...
    const videoKey = (new URLSearchParams(window.location.search).get('id') || '') + '_' + (currentVideoTitle || '');
    if (switchResourceCache.videoKey === videoKey && switchResourceCache.html) {
        container.innerHTML = switchResourceCache.html;
        return;
    }
    if (switchResourceCache.loading) {
        container.innerHTML = '<div class="text-center py-6 text-gray-400">正在加载资源列表...</div>';
        return;
    }
    container.innerHTML = '<div class="text-center py-6 text-gray-400">正在加载资源列表...</div>';
    loadSwitchResourceListInBackground();
...
// 进入页面时调用，后台加载资源列表并写入会话缓存；若当前展开区已打开则更新内容
function loadSwitchResourceListInBackground() {
    const urlParams = new URLSearchParams(window.location.search);
    const currentSourceCode = urlParams.get('source');
    const currentVideoId = urlParams.get('id');
    const videoKey = (currentVideoId || '') + '_' + (currentVideoTitle || '');
    if (switchResourceCache.videoKey === videoKey && switchResourceCache.html) return;
    if (switchResourceCache.loading) return;
    switchResourceCache.videoKey = videoKey;
    switchResourceCache.loading = true;
    ...
    (async () => {
        const resourceOptions = selectedAPIs.map((curr) => {
            ...
            if (customAPIs[customIndex]) return { key: curr, name: customAPIs[customIndex].name || '自定义资源' };
        ...
        await Promise.all(Object.entries(allResults).map(async ([sourceKey, result]) => {
            if (result) speedResults[sourceKey] = await testVideoSourceSpeed(sourceKey, result.vod_id);
        }));
        ...
        for (const [sourceKey, result] of sortedResults) {
            ...
            <div class="relative group ${isCurrentSource ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:scale-105 transition-transform'}" 
                ...
                ${isCurrentSource ? '<div class="absolute inset-0 flex items-center justify-center"><div class="bg-blue-600 bg-opacity-75 rounded-lg px-2 py-0.5 text-xs text-white font-medium">当前播放</div></div>' : ''}
            </div>`;
        }
        html += '</div>';
        switchResourceCache.html = html;
        switchResourceCache.loading = false;
        const expandEl = document.getElementById('switchResourceExpand');
        const contentEl = document.getElementById('switchResourceExpandContent');
        if (expandEl && expandEl.classList.contains('open') && contentEl) contentEl.innerHTML = html;
    })();
}
