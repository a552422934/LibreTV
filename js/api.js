        ...
        if (url.pathname === '/api/detail') {
            ...
            if (!API_SITES[sourceCode] && sourceCode !== 'custom') {
                return JSON.stringify({
                    ...
                    episodes: [],
                    detailUrl: '',
                    videoInfo: { title: '', source_name: '', source_code: sourceCode },
                    msg: '该数据源已移除或不可用'
                });
            }
...
async function testSiteAvailability(apiUrl) {
    try {
        // 必须带 source=custom，后端才会用 customApi 请求该地址；否则会走默认 source，测的不是当前源
        const response = await fetch('/api/search?wd=test&source=custom&customApi=' + encodeURIComponent(apiUrl), {
