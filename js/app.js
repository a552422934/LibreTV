...
let selectedAPIs = JSON.parse(
  // 默认选中资源
  localStorage.getItem("selectedAPIs") || '["dbzy","bfzy", "zy360"]'
);
...
function sanitizeSelectedAPIs() {
  const valid = selectedAPIs.filter((id) => {
    ...
    return (
      window.API_SITES &&
      Object.prototype.hasOwnProperty.call(window.API_SITES, id)
    );
  });
  ...
  try {
    ...
    for (const [key, info] of entries) {
      ...
      const span = document.querySelector(
        `#apiCheckboxes .api-status[data-api-status="${key}"]`
      );
      if (span) {
